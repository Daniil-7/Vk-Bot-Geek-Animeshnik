        ...
        # Для того чтобы бот читал все с маленьких букв 
          ...
          if message == 'привет':
            ...
            blasthack(id, 'Напиши "тянка" чтобы получить картинку тянки.')
              ...
              attachments = []
              image_url = 'https://www.thiswaifudoesnotexist.net/example-' + str(randint(0,99999)) + '.jpg'
              image = session.get(image_url, stream=True)
              photo = upload.photo_messages(photos=image.raw)[0]
              attachments.append( 'photo{}_{}'.format(photo['owner_id'], photo['id']) )
              give.messages.send( user_id=id, random_id=0, attachment=','.join(attachments), message='')
          else:
             blasthack(id, 'Я вас не понимаю! :( Напиши "тянка" чтобы получить картинку тянки.')
